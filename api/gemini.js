...
// Plain Vercel Node.js (sin Next.js, sin Edge Runtime)
// La key va en Vercel Environment Variables, no en el código
// ============================================================

export const maxDuration = 15; // 15s máximo — Gemini vision suele responder en 3-8s
...
export default async function handler(req, res) {
    ...
    // AbortController: matar la llamada a los 12s para no agotar el maxDuration
    ...
    try {
        ...
        if (!geminiRes.ok || data.error) {
            return res.status(200).json({ texto: null, error: data.error?.message || `Gemini HTTP ${geminiRes.status}` });
        }
        ...
        if (tipo === 'vision') {
            ...
            console.log(`Gemini vision: raw="${texto}" → clase=${clase}`);
